  ...
  .factory('User', function ($q, $rootScope, $firebaseSimpleLogin, $firebase, $location) {
    ...
    return {
      ...
      submit: function (method, data) {
        var fb
        if (auth.user) {
          fb = $firebase(new Firebase('https://well.firebaseio.com/' + method + '/' + auth.user.md5_hash))
        } else {
          var r = 'random' + Math.round( Math.random() * 100000 )
          console.log('User not logged in!!! Data recorded as ' + r)
          fb = $firebase(new Firebase('https://well.firebaseio.com/' + method + '/' + r))
        }
        fb.$add(data)
            ...
            return [
              {
                name: a,
                level: responses[a].level,
                diff: 4 > minmax ? minmax / 4 : 1
              },
              {
                name: b,
                level: responses[b].level,
                diff: 4 > minmax ? minmax : 4
              }
            ]
          } else if (a_b === .25) {
            return [
              {
                name: a,
                level: responses[a].level,
                diff: 8 > minmax ? minmax / 8 : 1
              },
              {
                name: b,
                level: responses[b].level,
                diff: 8 > minmax ? minmax : 8
              }
            ]
          } else if (a_b === 4) {
            return [
...
angular.module('well.controllers', [])
  ...
  .controller('FullCtrl', function ($scope, Full, Aspects) {
    $scope.aspects = {
      names: Aspects.names(),
      texts: Aspects.texts()
    }
