...
angular.module('well.services', [])
  .factory('User', function ($q, $rootScope, $firebaseSimpleLogin, $firebase, $location) {
    ...
    $rootScope.$on('$firebaseSimpleLogin:login', function(e, user) {
      ...
      // history.$loaded().then(function (values) {
      //   deferred.resolve()
      // })
    })
    ...
    return {
      ...
      submit: function (method, data) {
        ...
        if (auth.user) {
          ...
          var r = 'random' + Math.round( Math.random() * 100000 )
      ...
      data: function (method) {
        if (auth.user) {
          var fb = $firebase(new Firebase('https://well.firebaseio.com/' + method + '/' + auth.user.md5_hash))
          var data = fb.$asArray()
          return data.$loaded()
        } else {
  ...
  .factory('Aspects', function () {
    var names = ['Health', 'Family Happiness', 'Independence', 'Financial Security', 'Meaning & Value']
    var order = ['Health', 'Family Happiness', 'Independence', 'Financial Security', 'Meaning & Value']
    var texts = {
    ...
    var twenty = [
      ...
      'your and your family’s happiness',
      'your health (physical and mental)',
      'your morality (that is, living up to your personal values)',
      'your family relationships',
      'your financial security',
      'your options and possibilities in life',
      'your sense that your life is meaningful and has value',
      'how satisfied you are with your life',
      'your feeling that you have enough time and money',
      'how peaceful, calm, and harmonious your life is',
      'your feeling of independence and self-sufficiency',
      'your sense that you are making a difference in the world',
      'your feeling that the things you do are worthwhile',
      'your physical safety and security',
      'your being treated with dignity and respect by others',
      'how much of the time you feel happy',
      'your sense that things are getting better and better',
      'how happy you are overall with your life',
  ...
  .factory('Full', function (Aspects, Survey, User, $firebase) {
    var aspects = Aspects.names(),
      comparators = _.shuffle(aspects),
      responses = Survey.responded() ? Survey.responses() : Survey.randomize(),
      pairs = [],
      pears = [],
      paired = [],
    ...
    comparators = _.filter(comparators, function (a) { 
      return responses[a].level !== 100
    })

    var numeraire = comparators.pop()
    var n = (( comparators.length * (comparators.length - 1) ) / 2 ) - 1
    console.log(n)
    _.each(comparators, function (a) {
      _.each(comparators, function (b) {
        var p = [a, b].sort(),
          s = p[0] + p[1]

        if (a !== b && !_.contains(paired, s)) pears.push(p)
        paired.push(s)
      })
    })

    pairs = _.map(pears, function (p) {
      var s = _.shuffle(p)
      console.log(p, s)
      return s
    })
    ...
    _.each(aspects, function (a) {
      ...
      edges[a] = _.clone(edge)
    })

    console.log(aspects, numeraire, pairs)

    var positive = true
    console.log(positive)

    var bestValues = function (a, b, d_a, d_b) {
      var max_a = 100 - a
      var max_b = 100 - b
      if (positive) {
        if (max_a >= d_a && max_b >= d_b) {
          return [d_a, d_b]
        } else {
          if (a >= d_a && b >= d_b) {
            return [-d_a, -d_b]
          } else {
            if (max_a > max_b * (d_a / d_b)) {
              return [max_b * (d_a / d_b), max_b]
            } else if (max_b > max_a * (d_a / d_b)) {
              return [max_a, max_a * (d_a / d_b)]
            }
          }
        }
      } else {
    ...
    console.log('(100, 100, 4, 1)', bestValues(100, 100, 4, 1))
    console.log('(0, 0, 4, 1)', bestValues(0, 0, 4, 1))
    console.log('(99, 99, 4, 1)', bestValues(99, 99, 4, 1))
    console.log('(1, 1, 4, 1)', bestValues(1, 1, 4, 1))
    console.log('(1, 99, 4, 1)', bestValues(1, 99, 4, 1))
    console.log('(99, 1, 4, 1)', bestValues(99, 1, 4, 1))
    console.log('(0, 100, 4, 1)', bestValues(0, 100, 4, 1))
    console.log('(100, 0, 4, 1)', bestValues(100, 0, 4, 1))

    // var bestValues = function (max_a, max_b, d_a, d_b) {
    //   if (max_a > d_a && max_b > d_b) {
    //     return [d_a, d_b]
    //   } else if (max_a > d_a) {
    //     d_a = max_b * (d_a / d_b)
    ...
    //     return [max_a, d_b]
    //   } else {
    //     if (max_b * (d_a / d_b) > max_a) {
    //       return [max_a, max_a * (d_b / d_a)]
    //     } else {
    //       return [max_b * (d_a / d_b), max_b]
    //     }
    //   }
    ...
    var methods = {
      ...
      choice: function (p, a, b, d_a, d_b) {
        console.log(p, a, b, d_a, d_b)
        if (d_a < 0 && d_b < 0) {
          d_a = -d_a
          d_b = -d_b
          p = p === a ? b : a
        ...
        var r = Math.max(d_a, d_b) / Math.min(d_a, d_b),
          a_b = d_a / d_b

        if (a) {
          methods.path(p, a, b, a_b)
          methods.record(p, a, b, d_a, d_b)
        }
          ...
          return [
            {
              ...
              diff: bestValues(responses[a].level, responses[b].level, 4, 4)[0]
            },
            {
              name: b,
            ...
            return [
              {
                name: a,
                level: responses[a].level,
                diff: bestValues(responses[a].level, responses[b].level, 1, 4)[0]
              },
              {
                name: b,
                level: responses[b].level,
                diff: bestValues(responses[a].level, responses[b].level, 1, 4)[1]
              }
            ]
          } else if (a_b === .25) {
            return [
              {
                name: a,
                level: responses[a].level,
                diff: bestValues(responses[a].level, responses[b].level, 1, 8)[0]
              },
              {
                name: b,
                level: responses[b].level,
                diff: bestValues(responses[a].level, responses[b].level, 1, 8)[1]
              }
            ]
          } else if (a_b === 4) {
            return [
              {
                name: a,
                level: responses[a].level,
                diff: bestValues(responses[a].level, responses[b].level, 4, 2)[0]
              },
              {
                name: b,
                level: responses[b].level,
                diff: bestValues(responses[a].level, responses[b].level, 4, 2)[1]
              }
            ]
          }
        } else if (p === b) {
            ...
            return [
              {
                name: a,
                level: responses[a].level,
                diff: bestValues(responses[a].level, responses[b].level, 4, 1)[0]
              },
              {
                name: b,
                level: responses[b].level,
                diff: bestValues(responses[a].level, responses[b].level, 4, 1)[1]
              }
            ]
          } else if (a_b === .25) {
            return [
              {
                name: a,
                level: responses[a].level,
                diff: bestValues(responses[a].level, responses[b].level, 2, 4)[0]
              },
              {
                name: b,
                level: responses[b].level,
                diff: bestValues(responses[a].level, responses[b].level, 2, 4)[1]
              }
            ]
          } else if (a_b === 4) {
            return [
              {
                name: a,
                level: responses[a].level,
                diff: bestValues(responses[a].level, responses[b].level, 8, 1)[0]
              },
              {
                name: b,
                level: responses[b].level,
                diff: bestValues(responses[a].level, responses[b].level, 8, 1)[1]
              }
            ]
          }
        }
...
angular.module('well.controllers', [])
  ...
  .controller('FullCtrl', function ($scope, Full, Aspects, User) {
    $scope.aspects = {
      names: Aspects.names(),
      texts: Aspects.texts(),
    ...
    $scope.responses = Full.responses()
    // $scope.aspects.sorted = _.sortBy($scope.aspects.names, function (n) {
    //   return $scope.responses[n].level
    // })
    $scope.choice = Full.choice()
    $scope.weight = Full.weight

    $scope.choose = function (p, a, b, d_a, d_b) {
      $scope.choice = Full.choice(p, a, b, d_a, d_b)
      var neu = d3.select('.options').style('opacity', '0')
      var w = neu.style('width')
    ...
    $scope.$watch('choice', function (c) {
      if (!c) {
        $scope.weights = {}
        _.each($scope.aspects.names, function (a) {
          var w = Full.weight('Health', a)
          $scope.weights[a] = Math.sqrt(w[0]*w[1])
        })
        console.log($scope.weights)

        // var nonhunds = _.filter($scope.aspects.names, function (a) { 
        //   return $scope.responses[a].level !== 100
        // })
        // var hunds = _.filter($scope.aspects.names, function (a) { 
        //   return $scope.responses[a].level === 100
        // })
        ...
        console.log(sumW, $scope.weights)  

        $scope.score = _.reduce($scope.aspects.names, function (a, b) {
          return a + $scope.weights[b] * $scope.responses[b].level
        }, 0)

        User.data('full').then(function (data) {
          $scope.data = JSON.stringify(data[data.length - 1], null, 4)
        })
      }
    })
  })
