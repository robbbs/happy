...
"use strict";angular.module("well.directives",[]).directive("earth",["$window","Aspects",function(t){return{restrict:"EA",scope:{},link:function(e){window.onresize=function(){e.$apply()},e.$watch(function(){return angular.element(t)[0].innerWidth},function(){e.render()});var n=d3.select(".splash");e.render=function(){n.selectAll(".earth *").remove();{var t=window.innerWidth,r=window.innerHeight,i=d3.geo.orthographic().precision(0).scale(500).translate([t/2,r/2]).clipAngle(180),a=d3.select(".splash .earth").append("canvas").attr("width",t).attr("height",r),l=a.node().getContext("2d"),o=d3.geo.path().projection(i).context(l);d3.random.logNormal(1)}_.each(e.responses,function(t){t.rand=20*Math.random()+10}),d3.json("world-110m.json",function(e,n){function a(){i.rotate([.1*d,.1*c,30]),l.clearRect(0,0,t,r),_.each(s,function(t){l.beginPath(),l.strokeStyle=d3.hsl(t.properties.color*d,1,.8).toString(),o(t),l.stroke()})}var d,c=0;window.onscroll=function(){c=(window.pageYOffset||document.scrollTop)-(document.clientTop||0)||0,a()},d3.select("body").on("mousemove",function(){d=d3.mouse(this)[0],a()});var s=(topojson.feature(n,n.objects.land),topojson.feature(n,n.objects.countries).features);_.each(s,function(t){t.properties.color=Math.random()}),a()})}}}}]).directive("range",["$window","$timeout","Aspects",function(t,e){return{restrict:"EA",scope:{level:"="},link:function(n,r){var i=d3.select(r[0]).append("svg").style("width","100%").style("height",22);window.onresize=function(){n.$apply()},n.$watch(function(){return angular.element(t)[0].innerWidth},function(){n.render()});var a=null;n.$watch("level",function(t){t!==a&&n.render()}),n.render=function(){function t(){var t=c.extent()[0],r=d.invert(d3.mouse(this)[0]);t=isNaN(r)?t:r,c.extent([t,t]),t&&(t=Math.round(t)),v.style("display","block").attr("d",h(p().set(d(t)))+"Z"),a=t,e(function(){n.level=t})}i.selectAll("*").remove();var l=d3.select(r[0])[0][0].offsetWidth,o=22,d=d3.scale.linear().domain([0,100]).range([6,l-6]).clamp(!0),c=d3.svg.brush().x(d).extent([n.level,n.level]).on("brush",t),s=i.append("g").attr("class","axis");s.call(d3.svg.axis().scale(d).orient("bottom").tickSize(0).tickValues([]));var f=s.select(".domain").attr("transform","translate(0, 10)");f.select(function(){return this.parentNode.appendChild(this.cloneNode(!0))}).attr("class","halo");var u=i.append("g").attr("class","slider").call(c);u.selectAll(".extent,.resize").remove(),u.select(".background").attr("height",o);var p=function(){var t=d(1),e=10;return{set:function(n){return t=n,[{x:t-6,y:e-6},{x:t+6,y:e-6},{x:t+6,y:e+6},{x:t,y:e+10},{x:t-6,y:e+6}]}}},h=d3.svg.line().x(function(t){return t.x}).y(function(t){return t.y}),v=u.append("path").attr("class","handle").attr("d",h(p().set(d(n.level)))+"Z").style("display","none")}}}}]).directive("bar",["$window","Aspects","$filter",function(t,e,n){return{restrict:"E",scope:{level:"=",diff:"=",name:"=",choice:"="},link:function(r,i){var a=d3.select(i[0]).append("svg").style("width","100%").style("height",40);window.onresize=function(){r.$apply()},r.$watch("level",function(){r.render()}),r.$watch("diff",function(){r.render()}),r.$watch("name",function(){r.render()}),r.$watch("choice",function(){r.render()},!0),r.$watch(function(){return angular.element(t)[0].innerWidth},function(){r.render()});var l=0,o=0;r.render=function(){a.selectAll("*").remove();var t=d3.select(i[0])[0][0].offsetWidth,d=t>0?t:100,c=40,s=d3.scale.linear().domain([0,100]).range([6,d-6]),f=a.append("g").attr("class","axis").attr("transform","translate(0,"+(c-20)+")");f.call(d3.svg.axis().scale(s).orient("bottom").tickSize(0).tickPadding(12)),f.select(".domain").remove();var u=(a.append("rect").attr("x",s(0)).attr("y",4).attr("height",20).attr("width",s(100)-s(0)).style("fill",e.getColor(r.name,-1)),a.append("rect").attr("x",s(0)).attr("y",4).attr("height",20).attr("width",s(r.level)-s(0)).style("fill",e.getColor(r.name,100)),a.append("rect").attr("y",4).attr("height",20).style("fill",e.getColor(r.name,205))),p=function(t,e){return"M"+s(t)+" 3 L"+Number(s(e)-4)+" 3L"+Number(s(e)-4)+" 1 L"+s(e)+" 4L"+Number(s(e)-4)+" 7L"+Number(s(e)-4)+" 5L"+s(t)+" 5 Z"},h=function(t,e){return"M"+s(t)+" 3 L"+Number(s(e)+4)+" 3L"+Number(s(e)+4)+" 1 L"+s(e)+" 4L"+Number(s(e)+4)+" 7L"+Number(s(e)+4)+" 5L"+s(t)+" 5 Z"},v=a.append("path").attr("stroke","none").style("fill","#000"),w=a.append("text").attr("y",14).attr("dy",".375em").style("fill","#000").style("font-weight","400"),g=a.append("line").attr("x1",s(r.level+r.diff)).attr("y1",4).attr("x2",s(r.level+r.diff)).attr("y2",24).attr("class","hidden").style("stroke","#000");if(r.diff>0){o=r.diff,l=s(r.diff)-s(0),u.attr("x",s(r.level)).attr("width",0).transition().delay(300).duration(1e3).attr("width",l).each("end",function(){g.attr("class","")}),v.attr("d",p(r.level,r.level)).transition().delay(300).duration(1e3).attr("d",p(r.level,r.level+r.diff)),w.text("+"+n("round1")(r.diff));var t=w.style("width").replace("px","");s(100)-s(r.level)-s(r.diff)>t?w.attr("x",s(r.level)+s(r.diff)).attr("dx",4):w.attr("x",s(100)-t).attr("dx",-4)}else r.diff<0?(o=r.diff,l=s(-r.diff)-s(0),u.attr("x",s(r.level)).attr("width",0).style("fill",e.getColor(r.name,40)).transition().delay(300).duration(1e3).attr("x",s(r.level)-l).attr("width",l).each("end",function(){g.attr("class","")}),v.attr("d",h(r.level,r.level)).transition().delay(300).duration(1e3).attr("d",h(r.level,r.level+r.diff)),w.text(n("round1")(r.diff)),w.attr("x",s(r.level)+s(r.diff)).attr("dx",4)):o>0?(l=s(o)-s(0),u.attr("x",s(r.level)).attr("width",l).transition().duration(1e3).attr("width",0).each("end",function(){g.attr("class","")})):0>o&&(l=s(-o)-s(0),u.attr("x",s(r.level)-l).attr("width",l).style("fill",e.getColor(r.name,40)).transition().delay(300).duration(1e3).attr("x",s(r.level)).attr("width",0).each("end",function(){g.attr("class","")}))}}}}]).directive("fix",["$window",function(t){return{restrict:"EA",scope:{},link:function(e,n){var r=d3.select(n[0]),i=d3.select(r[0][0].parentNode);window.onresize=function(){e.$apply()},e.$watch(function(){return angular.element(t)[0].innerWidth},function(){e.render()}),e.render=function(){var t=i[0][0].getBoundingClientRect();console.log(t,window.scrollLeft),r.style({position:"fixed",padding:i.style("padding"),top:window.scrollTop?t.top+window.scrollTop+"px":t.top+"px",left:window.scrollLeft?t.left+window.scrollLeft+"px":t.left+"px",width:t.width+"px"})}}}}]).directive("prog",["$window",function(t){return{restrict:"EA",scope:{complete:"="},link:function(e,n){var r=d3.select(n[0]).append("svg").style("width","100%").style("height",8);window.onresize=function(){e.$apply()},e.$watch(function(){return angular.element(t)[0].innerWidth},function(){e.render()}),e.$watch("complete",function(){e.render()}),e.render=function(){r.selectAll("*").remove();var t=r[0][0].getBoundingClientRect();r.append("rect").attr("height",4).attr("width",t.width*e.complete).attr("fill","#559")}}}}]);
