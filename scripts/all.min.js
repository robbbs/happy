...
angular.module('well.directives', [])
  .directive('slider', function($interpolate, $window, $timeout, Aspects, Survey, TradeOff) {
    return {
      restrict: 'EA',
      template: '<span class="pull-right">{{ level + diff }}</span><h5>{{ text }}</h4><div class="svg" ng-class="{ unfilled: !level }"></div>',
  ...
  directive('plot', function ($window) {
    return {
      restrict: 'EA',
      scope: {
        data: '=',
