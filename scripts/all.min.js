...
.config(function ($routeProvider, $locationProvider) {
  $routeProvider
    ...
    .when('/threetwo', {
      templateUrl: 'threetwo.html',
      controller: 'ThreeTwoCtrl'
    })
    .when('/tradeoff', {
...
angular.module('well.tradeoff', [])
  ...
  // --------------------------------------------------
  //
  .controller('ThreeTwoCtrl', function ($scope, Aspects, Survey, ThreeTwo, User, $location) {
    $scope.aspects = {
      names: Aspects.names(),
      texts: Aspects.texts(),
    ...
    $scope.responses = Survey.responses()
    $scope.choice = ThreeTwo.choice()

    $scope.choose = function (i) {
      $scope.choice = ThreeTwo.choice(i)
      ...
      choices.style('pointer-events', 'none')
      ...
      old
        .transition()
          ...
          .each('end', function () {
            old.remove()
            choices.style('pointer-events', 'auto')
          })
  ...
  .controller('CompleteCtrl', function ($scope, Aspects, Survey, User) {
    $scope.aspects = {
      names: Aspects.names(),
      texts: Aspects.texts(),
      color: Aspects.getColor
  ...
  .factory('Tradeoff', function () {
    // 2 * i + 1 || 2 * i + 2
    var positive_tradeoffs = [         
                            [4,4],
              [1,4],                   [4,1],
        [1,8],       [2,4],      [4,2],      [8,1],
      // MRS based on final tradeoff where each number represents the MRS of X in terms of Y as a power of 2
      -4,  -2.5,  -1.5,  -0.5, 0.5,  1.5,  2.5,  4
    ]

    var negative_tradeoffs = [         
                          [-4,-4],
            [-4,-1],                 [-1,-4],
        [-8,-1],    [-4,-2],    [-2,-4],   [-1,-8],
      -4,  -2.5,  -1.5,  -0.5, 0.5,  1.5,  2.5,  4
    ...
    // corresponds to the last tradeoff decision i - 7
    var tradeoff_mapping = [0.0625, 0.1875, 0.375, 0.75, 1.5, ]

    // where a and b are the rating levels for aspects a and b
    var checkCorners = function (a, b) {
      if (a > 92 && b < 8) {
        return 'skip'
      } else if (a < 8 && b > 92) {
        return 'skip'
    ...
    var methods = {
      // where a and b are the rating levels for aspects a and b
      values: function (a, b, sign) {
        sign = checkCorners(a, b) === null ? sign : checkCorners(a, b)
        if (sign === 'positive') {
          return positive_tradeoffs
        } else if (sign === 'negative') {
          return negative_tradeoffs
        } else {
          return 'skip'
        }
      }
  ...
  .factory('ThreeTwo', function (Aspects, Survey, Tradeoff, User) {
    var aspects = Aspects.names()
    var responses = Survey.responded() ? Survey.responses() : Survey.randomize()

    var choices = {}
    choices.levels = responses
    choices.positive = Math.random() > 0.5 ? 'positive' : 'negative'
    choices.skipped = []
    choices.decisions = []
    choices.MRS = {}
    ...
    choices.record = function (aspects, i) {
      this.decisions.push({
        aspects: aspects,
        i: i,
        t: Date.now()
      ...
      if (i > 6) {
        this.MRS[aspects[0]][aspects[1]] = Tradeoff.values(responses[aspects[0]].level, responses[aspects[1]].level, choices.positive)[i]
      }
    }
    ...
    // first tradeoff three aspects against each, as explained in Ipsos instructions
    var three = {}
    ...
    three.pairs = _.filter(three.pairs, function (pair) {
      var skip = Tradeoff.values(responses[pair[0]].level, responses[pair[1]].level, choices.positive) === 'skip'
      if (skip) choices.skipped.push(pair)
      return !skip
    })

    // tradeoff leftover two aspects
    ...
    var numberOfQs = (three.pairs.length + two.pairs.length) * 3
    var step = 0
    var pair = {}
    ...
    var methods = {
      choice: function (i) {
        if (!i) {
          i = 0
          pair = three.pairs.pop()
        } else {
          choices.record(pair, i)
        }
        ...
        return {
          aspects: pair,
          tradeoff: Tradeoff.values(responses[pair[0]].level, responses[pair[1]].level, choices.positive)[i],
          i: i,
          completed: step++ / numberOfQs
        }
      },
      weights: function () {
        return [
