...
angular.module('well.tradeoff', [])
  ...
  .controller('ThreeCtrl', function ($scope, Choices, Three, Two, Slide, $location) {
    ...
    $scope.$watch('choice', function (c) {
      $scope.choices = Choices
      var total = $scope.settings.overlap ? Three.numberOfQs + Two.numberOfQs + 3 : Three.numberOfQs + Two.numberOfQs
      $scope.completed = Choices.step++ / total
      if (!c) {
        if ($scope.settings.overlap) {
  ...
  .factory('Two', function (Aspects, Survey, Tradeoff, Choices, Three) {
    ...
    two.pairs = [ [aspects[3], aspects[0]], [aspects[4], aspects[0]] ] 
