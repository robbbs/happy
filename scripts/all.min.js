...
angular.module('well.directives', [])
  .directive('slider', function($window, $timeout, Aspects, Survey, TradeOff) {
    return {
      ...
      template: '<h5><span>{{ text }}:</span><span class="pull-right">{{ level + diff }}</span></h5><div class="svg" ng-class="{ unfilled: !level }"></div>',
        ...
        scope.render = function () {
          ...
          function brushed() {
            ...
            if (!isNaN(scope.diff) && scope.diff !== 0) {
              ...
              d3.select(element[0].children[1]).style('background-color', Aspects.getColor(scope.name, value + scope.diff))
            } else if (value) {
              ...
              d3.select(element[0].children[1]).style('background-color', Aspects.getColor(scope.name, value))
            }
            if (scope.diff > 0) {
              rect
                  .attr('class', 'plus')
      ...
      link: function (scope, element, attrs) {
        ...
        var multi = function () {
          ...
          svg.selectAll('.weight')
            .data(seriesArr)
            .enter().append('text')
              .text(function (d) { return '(' + d.values[0].weight.toFixed(2) + ')' })
              .attr('class', 'weight')
              .attr('text-anchor', 'end')
              .attr('x', function (d) { return x(d.values[0].label) })
              .attr('dx', -10)
