...
angular.module('well.services', [])
  .factory('User', function ($q, $rootScope, $location, $timeout) {
    ...
    user.info = user.firebase.getAuth()
    ...
    user.addData = function (module, data) {
      this.firebase.onAuth(function () {
        user.firebase.child('module/' + module + '/' + user.info.uid).push(data)
      })
    }
    ...
    user.getData = function (module, limit) {
      var deferred = $q.defer()
      this.firebase.onAuth(function () {
        user.firebase.child('module/' + module + '/' + user.info.uid).on('value', function (snapshot) {
...
angular.module('well.overlap', [])
  .factory('Overlap', function (Three, Choices, Tradeoff, Survey, Aspects) {
    var responses = Survey.responses()
    var overlap = {}
    overlap.aspects = Three.aspects
    console.log(overlap.aspects, Three.aspects, Aspects.names())
    overlap.MRS = {}
        ...
        User.getData('threetwo').then(function (response) {
          data = _.map(response, function (datum) {
            var score = _.reduce(datum.aspects, function (memo, num) {
              return memo + datum.responses[num].level * datum.weights[num]
            }, 0)
            return {
              score: score,
              date: datum.date
