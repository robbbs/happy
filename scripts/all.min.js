...
angular.module('well.controllers', [])
  .controller('AppCtrl', function ($scope, User, $location) {
    ...
    $scope.colors = d3.range(5).map(function(d) { return d3.hsl((d + 6) * 70, 1, .8).toString() })
    $scope.colors = [
      '#ffff99',
      '#99ffaa',
      '#99ddff',
  ...
  .controller('TradeOffCtrl', function ($scope, Aspects, TradeOff) {
    ...
    $scope.prefs = {
      ...
      next: function () {
        ...
        $scope.choose(this.range[this.degree])
        this.degree = 0
      }
    }
  })
      ...
      for (var i in keys) {
        ...
        for (var a in responses) {
          ...
          if (responses[a].lower && responses[a].upper) {
            ...
            console.log(responses[a].lower, responses[a].upper, weight)
          } 
          datum[a] = {
  ...
  .controller('TestCtrl', function ($scope, $routeParams) {
    ...
    var w = {
      ...
      'Happiness': logNormal(),
      'Freedom': logNormal(),
      'Financial Security': logNormal(),
      'Meaning/Value': logNormal()
    }
...
angular.module('well.directives', [])
  .directive('slider', function($window, $timeout, Aspects, Survey, TradeOff) {
    return {
      restrict: 'EA',
      template: '<span class="pull-right"><b>{{ level + diff }}</b></span><h5>{{ text }}:</h5><div class="svg" ng-class="{ unfilled: !level }"></div>',
          ...
          function brushed() {
            ...
            if (!isNaN(scope.diff) && scope.diff !== 0) {
              handle
                ...
                .transition()
                  .duration(250)
                  .attr('d', line( pointy().set( x(value) + x(scope.diff) ) ) + 'Z')

              d3.select(element[0].children[2]).style('background-color', Aspects.getColor(scope.name, value + scope.diff))
            } else if (value) {
              ...
              d3.select(element[0].children[2]).style('background-color', Aspects.getColor(scope.name, value))
            }
            if (scope.diff > 0) {
              rect
                  .attr('class', 'plus')
      ...
      link: function (scope, element, attrs) {
        ...
        var single = function () {
          ...
          svg.selectAll('.value')
            .data(seriesArr)
            .enter().append('text')
              .text(function (d) { return '(' + d.values[0].weight.toFixed(2) + ') ' + d.values[0].value })
              .attr('class', 'value')
        ...
        var multi = function () {
          ...
          values.selectAll('text').
            ...
            .enter().append('text')
              .attr('class', 'value')
              .text(function (d) {
                return '(' + d.weight.toFixed(2) + ') ' + d.value
              })
              .attr('x', function (d) { return x(d.label) })
              .attr('y', function (d) { return y(d.y0 + (d.y / 2)) })
              .attr('text-anchor', function (d) { 
                if (d.label === dates[0]) return 'start'
                else if (d.label === dates[dates.length - 1]) return 'end'
                else return 'middle'
              })
              .attr('dx', function (d) {
                if (d.label === dates[0]) return 10
                else if (d.label === dates[dates.length - 1]) return -10
                else return 0             
              })
          ...
          svg.append("g")
            ...
            .selectAll('text')
              .style('text-anchor', function (d) {
                if ((new Date(dates[0])).getTime() === d.getTime()) return 'start'
                else if ((new Date(dates[dates.length - 1])).getTime() === d.getTime()) return 'end'
                else return 'middle'
              })
        }
