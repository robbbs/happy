...
"use strict";angular.module("well.survey",[]).controller("ResultsCtrl",["$scope","Aspects","Survey","Choices","User",function(e,s,t,n,a){if(e.weights=n.weights(),e.aspects.sorted=_.sortBy(e.aspects.names,function(s){return-1*e.weights[s]}),e.data=a.getData("threetwo"),!e.weights){e.weights={},_.each(e.aspects.names,function(s){e.weights[s]=Math.random()}),console.log(e.weights);var r=0;_.each(e.aspects.names,function(s){r+=e.weights[s]}),_.each(e.aspects.names,function(s){e.weights[s]=e.weights[s]/r})}}]).controller("SurveyCtrl",["$scope","Aspects","Survey","Style","$location","$routeParams","Trans",function(e,s,t,n,a){t.reset(),s.names().then(function(s){e.aspects.names=s,t.aspects=s}),e.survey={finished:!1},e.$watch("responses",function(){t.responded()&&(e.survey.finished=!0)},!0),e.next=function(){switch(a.path().split("/")[1]){case"threetwo":a.path("threetwo/abbcca");break;case"overlap":a.path("overlap/abac");break;case"fineoverlap":a.path("fineoverlap/tradeoff")}}}]).controller("TransCtrl",["$scope","Aspects","Survey","Style","$location","$routeParams",function(e,s,t){s.names().then(function(s){e.aspects.names=s,t.aspects=s})}]).factory("Survey",["Aspects","$q",function(e,s){var t=this,n=this;return this.responses={},this.getResponses=function(){var a=s.defer();return t.aspects?a.resolve(n.responses):e.names().then(function(e){n.aspects=e,_.each(e,function(e){n.responses[e]||(n.responses[e]=null)}),a.resolve(n.responses)}),a.promise},this.respond=function(e,s){return t.responses[e]=s},this.responded=function(){return t.aspects&&_.every(n.aspects,function(e){return n.responses[e]})},this.randomize=function(){console.log("randomized");var a=s.defer();return t.getResponses().then(function(){e.names().then(function(e){for(var s in e)n.responses[e[s]]=Math.max(Math.ceil(99*Math.random()),Math.max(Math.ceil(99*Math.random()),Math.ceil(99*Math.random())));a.resolve(n.responses)})}),a.promise},this.safeRandomize=function(){return t.responded()?t.getResponses():t.randomize()},this.reset=function(){return t.aspects&&_.each(n.aspects,function(e){n.responses[e]=null}),t.responses},this.minMax=null,this.range=function(){var e=s.defer();return t.minMax?e.resolve(t.minMax):t.safeRandomize().then(function(s){var n=_.map(s,function(e){return e}),a=Math.max(0,_.min(n)-8),r=Math.min(100,_.max(n)+8);t.minMax=[a,r],console.log(t.minMax),e.resolve(t.minMax)}),e.promise},this}]).factory("Trans",function(){var e=this;return this.ition=function(){d3.selectAll(".slide range").style("opacity",1).transition().duration(300).style("opacity",0).each("end",function(){d3.selectAll(".slide range").remove()}),d3.select(".panel").remove();var e=d3.select("ul.survey");e.style("width","100%").transition().delay(300).duration(1e3).style("width","50%").each("end",function(){var s=e.select(function(){return this.parentNode.appendChild(this.cloneNode(!0))});s.style("opacity",0).transition().duration(500).style("opacity",1)})},e});
